...
- 특수 변형(specialization) — 제품·목적이 그 구조를 요구할 때만:
  ...
  - **`problem-solution`**: 페인포인트 스토리텔링(고민→공감→해결→결과, PAS)이 핵심일 때 → 같은 인물·부위로 problem/result 일관.
...
## 규칙
- **이미지 본 생성은 반드시 `gen_image.sh` 경유**(ad-image 위임). `higgsfield generate create text2image_*` 직접 호출은 페르소나 서브게이트 샘플 외 금지 — 우회 시 제품 레퍼런스·필수 구문·보드·QA가 모두 빠진다.
